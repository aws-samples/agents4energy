...
export const aguiHandlerSchema = a.schema({
  ...
  // Amplify owns the schema field (so AppSync knows the type); the resolver
  // itself is escape-hatched to a real HTTP resolver in backend.ts (reads
  // ctx.env.AGUI_RUNTIME_ARN).
  invokeHandler: a
    .mutation()
    .arguments({
      sessionId: a.string().required(),
