#!/usr/bin/env node
import { AgentCoreStack } from '../lib/cdk-stack.js';
...
/**
 * Reads amplify_outputs.json from the web/ directory (sibling to agent/) and
 * returns values from the `custom` section. Returns an empty object if the
 * file doesn't exist or hasn't been deployed yet (non-fatal).
 */
function readAmplifyOutputs(projectRoot: string): Record<string, string> {
  const amplifyOutputsPath = path.resolve(projectRoot, '..', 'web', 'amplify_outputs.json');
  try {
    const outputs = JSON.parse(fs.readFileSync(amplifyOutputsPath, 'utf8'));
    return (outputs?.custom as Record<string, string>) ?? {};
  } catch {
    return {};
  }
}
...
async function main() {
  ...
  const amplifyAuthRoleArn = amplifyOutputs.auth_authenticated_role_arn;
